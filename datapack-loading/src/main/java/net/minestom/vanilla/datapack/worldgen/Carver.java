...
public record Carver(Key type, BaseConfig config) {
    ...
    // cave - Carves a cave.
    // A cave is a long tunnel that sometimes branches. Sometimes one or more tunnels start from a circular void.
    //
    // nether_cave - Similar to cave, but with a less frequency and wider tunnels. And aquifer doesn't work.
    // The carved blocks below bottom_y + 32.0 are filled with lava.
    //
    ...
    public interface BaseConfig {
        ...
        // lava_level: The Y-level below or equal to which the carved areas are filled with lava.
        // Doesn't affect nether_cave (where lava level is always bottom_y + 31).
        // (This field is seemingly ignored and always set to -56 (MC-237017), needs testing)
        HeightProvider lava_level();

        // replaceable: Blocks that can be carved.
        // Can be a block ID, a block tag, or a list of block IDs/tags.
        JsonUtils.SingleOrList<Key> replaceable();

        // debug_settings: (optional) Replaces blocks in the carved areas for debugging.
        //
        // debug_mode: (optional, defaults to false) Enable debug mode for this carver.
        ...
        @Optional DebugSettings debug_settings();

        record DebugSettings(
                @Optional Boolean debug_mode,
                @Optional Block air_state,
                @Optional Block water_state,
                @Optional Block lava_state,
    ...
    public record Config(
            ...
            HeightProvider lava_level,
            JsonUtils.SingleOrList<Key> replaceable,
            @Optional BaseConfig.DebugSettings debug_settings
    ...
    // floor_level: Value between -1.0 and 1.0 (both inclusive).
    // Change the shape of the cave's horizontal floor.
    // If 0.0, carves the terrain with ellipsoids.
    // If 1.0, carves with upper semi-ellipsoids, resulting in a level floor.
    public record CaveConfig(
            float probability,
            HeightProvider y,
            HeightProvider lava_level,
            JsonUtils.SingleOrList<Key> replaceable,
            ...
            FloatProvider vertical_radius_multiplier,
            FloatProvider floor_level
    ) implements BaseConfig {
    }
    ...
    public record CanyonConfig(
            ...
            @Optional BaseConfig.DebugSettings debug_settings,
            FloatProvider yScale,
            FloatProvider vertical_rotation,
            Shape shape
    ) implements BaseConfig {
